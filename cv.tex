...
\cvline{Outils et produits}{Ant, Maven, Jenkins, Tomcat, JBoss AS, Apache2, ActiveMQ, Git, SVN, CVS, YourKit, JProfiler, JMeter, MAT}
...
\cventry{Septembre 2013 -- aujourd'hui}{Mirakl}{Paris}{Développeur Java}{}{
  \begin{itemize}
    ...
    \resitem{\textit{Sujets :} Divers refactorings techniques dans le cadre d'un chantier de restructuration de l'application (évolutions et maintenances devenues difficiles, performances bientôt problématiques). Notamment, migration du modèle de données persistant de Grails (GORM) vers Hibernate (+ QueryDSL), avec les adaptations que cela implique : écriture de repositories, transposition de la validation, etc. Travail d'analyse (profiling) et de résolution de problèmes de performances émergents liés aux volumes croissants de données. Soutien technique et aide à la conception auprès des autres membres de l'équipe.}
    \resitem{\textit{Technologies et outils :} Grails 2.3 (+ plugin JAX-RS), Spring 3 (AOP, ORM, Security, Batch), Hibernate 3, Hibernate Validator 5.0, Mysema QueryDSL, Logback, JUnit, Mockito, Tomcat 7, PostgreSQL 9.2 puis 9.3, Git, Maven, Jenkins, MemoryAnalyzerTool, JProfiler, YourKit.}
  \end{itemize}
}
